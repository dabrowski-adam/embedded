...
void init_timer ()
{
    ...
    PWMPR = 0;                      // No Prescalar
    PWMMCR = 0x0002;                // Reset on MR0: the PWMTC will be reset if MR0 matches it
    PWMPCR = 0x200;                 // Enable PWM1 ouput
    PWMMR0 = 100;                   // Dummy write in the PWM0 match register
    PWMMR1 = 50;                    // Dummy write in the PWM1 match register
    PWMTCR = 2;                     // Counting enable, PWM enabled
    PWMTCR = 9;                     // Counting enable, PWM enabled
}
...
void set_dac_sample_rate(unsigned int sample_rate)
{
  unsigned int _T0MCR;
    ...
    PWMLER = 3;
    PWMMR0 = _T0MCR;                // Define PWM frequency
    PWMMR1 = _T0MCR >> 1;           // Define 50% of duty cycle
}

void init_IO(void)
{
  PINSEL0 = 0x000000002;      /* PWM1 out */
