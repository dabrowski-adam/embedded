...
void testADC(void *pvParameters)
{
	...
	//  PINSEL1 |=  0x15080000;
  	...
  	ADCR = (1 << 0)                             |  //SEL = 1, dummy channel #1
            ((configCPU_CLOCK_HZ /
       configVPBDIV_FACTOR) / 4500000 - 1) << 8 |  //set clock division factor, so ADC clock is 4.5MHz
...
//    printf("\rAIN1: %d = %d mV", integerResult, mvResult);
    	...
    	printf("\rAIN1: ");
    	SendNumber(10,4,FALSE,' ', integerResult);
    	printf(" = ");
    	SendNumber(10,4,FALSE,' ', mvResult);
    	printf(" mV");
...
//    printf(", DAC = %d", dacValue);
	    printf(", DAC = ");
    	SendNumber(10,4,FALSE,' ', dacValue);
